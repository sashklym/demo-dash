...
#
# Coolify runs on a private network, so deploys are triggered from a machine that
# can reach it — not from public CI. Coolify itself pulls the public GitHub repo
# and builds the Dockerfiles.
#
...
# Coolify application UUIDs (resource ids, not secrets). Override via env if needed.
